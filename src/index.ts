...
import { add_load, delete_load,  get_loads } from "./services/load";
...
function main(){
    if(!TELEGRAM_BOT_API){
        return console.log("Sorry you didn't provide a telegram bot api")
    }
    if(!mongourl){
        return console.log("Sorry you didn't provide a mongo url api api")
    }
    if(!owner_id){
        return console.log("Sorry you didn't provide an owner id")
    }
    console.log("Env variables provided successfully")
        ...
        try {
            ...
            switch (data) {
                ...
                case "show_loads": {
                    ...
                    await bot.sendMessage({ chat_id, text: "Loads: " });
                    for (const processed_load of processed_loads) {
                        await bot.sendMessage({ chat_id, text: processed_load });
                    }
                    await bot.sendMessage({ chat_id, text: "----------" });
                    actor.send({ type: "START_COMMAND" });
                ...
                case "show_batteries": {
                    const batteries = await get_all_batteries();
                    if (!batteries) {
                        await bot.sendMessage({ chat_id, text: "Sorry component not found" });
                        break;
                    }
                    const processed_batteries = process_components(batteries, "battery")!;
                    await bot.sendMessage({ chat_id, text: "----------" });
                    await bot.sendMessage({ chat_id, text: "Batteries: " });
                    for (const processed_battery of processed_batteries) {
                        await bot.sendMessage({ chat_id, text: processed_battery });
                    }
                    await bot.sendMessage({ chat_id, text: "----------" });
                    actor.send({ type: "START_COMMAND" });
                    return bot.answerCallbackQuery({ callback_query_id: query.id });
                ...
                case "show_pvs": {
                    ...
                    await bot.sendMessage({ chat_id, text: "----------" });
                    await bot.sendMessage({ chat_id, text: "PVs: " });
                    const processed_pvs = process_components(pvs, "pv")!;
                    for (const processed_pv of processed_pvs) {
                        await bot.sendMessage({ chat_id, text: processed_pv });
                    }
                    await bot.sendMessage({ chat_id, text: "----------" });
                    actor.send({ type: "START_COMMAND" });
                    return bot.answerCallbackQuery({ callback_query_id: query.id });
            ...
            if (keyboard.length === 0) {
                await bot.editMessageText({ message_id, chat_id, text });
            } else {
                await bot.editMessageText({ message_id, chat_id, text, reply_markup });
            }
            return bot.answerCallbackQuery({ callback_query_id: query.id });
        } catch (error) {
            console.error(error);
